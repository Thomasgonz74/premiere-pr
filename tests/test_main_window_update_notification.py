...
def test_clicking_download_triggers_a_verified_download_instead_of_opening_a_browser(window):
    win, settings, update_checker = window
    with _simulate_click(0):  # "Télécharger"
        with patch.object(update_checker, "download_verified_installer") as mock_download:
...
def test_verification_failure_falls_back_to_opening_the_release_page(window):
    win, settings, update_checker = window
    with _simulate_click(0):  # "Télécharger"
        with patch.object(update_checker, "download_verified_installer"):
            win._on_update_available("v2.0.0", "https://example.com/releases/v2.0.0")

    with patch("torrent2000.ui.main_window.QDesktopServices.openUrl") as mock_open:
        update_checker.installer_verification_failed.emit("no digest published")
...
def test_verified_installer_is_launched_only_after_an_explicit_yes_click(window):
    win, settings, update_checker = window
    with patch("torrent2000.ui.main_window.subprocess.Popen") as mock_popen:
        with _simulate_click(1):  # "Plus tard" on the second (launch) dialog
            update_checker.installer_verified.emit(r"C:\fake\Torrent2000-Setup.exe")
        mock_popen.assert_not_called()

        with _simulate_click(0):  # explicit "Lancer" click
            update_checker.installer_verified.emit(r"C:\fake\Torrent2000-Setup.exe")
        mock_popen.assert_called_once_with([r"C:\fake\Torrent2000-Setup.exe"])


def test_launching_the_installer_closes_the_running_app(window):
    # Torrent2000.exe stays locked by Windows while this process is running,
    # and the installer needs to overwrite that exact file -- launching it
    # without closing first would make the update silently fail to apply.
    win, settings, update_checker = window
    with patch("torrent2000.ui.main_window.subprocess.Popen"):
        with patch.object(win, "close") as mock_close:
            with _simulate_click(0):  # "Lancer"
                update_checker.installer_verified.emit(r"C:\fake\Torrent2000-Setup.exe")
